...
const { QraphQLError, GraphQLBoolean, GraphQLError } = require("graphql");
...
const resolvers = {
  Query: {
    ...
    allAuthors: () => {
      const toReturn = Array.from(
        ...
        return {
          name: authorName,
          bookCount: books.filter((b) => b.author === authorName).length,
          born: books.find((b) => b.author === authorName).born,
        };
      });
      return toReturn;
    },
  ...
  Mutation: {
    ...
    editAuthor: (root, args) => {
      const editingAuthor = books.find((book) => book.author === args.name);
      if (!editingAuthor) return null;

      books = books.map((book) =>
        book.author !== args.name ? book : { ...book, born: args.setBornTo }
      );
